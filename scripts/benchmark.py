#!/usr/bin/env python3
"""
Skrypt do benchmarkingu programu obliczającego liczbę PI
Uruchamia program z różnymi konfiguracjami i generuje wykres wydajności
"""
...
def run_pi_calculator(intervals, threads):
    ...
    """
    try:
        # Przygotowanie wejścia dla programu
        ...
        # Parsowanie wyniku - szukamy czasu wykonania
        match = re.search(r'Czas obliczeń:\s+([\d.]+)\s+sekund', result.stdout)
        if match:
            return float(match.group(1))
        else:
            print(f"Błąd parsowania dla {intervals} przedziałów i {threads} wątków")
            return None
            
    except subprocess.TimeoutExpired:
        print(f"Timeout dla {intervals} przedziałów i {threads} wątków")
        return None
...
def main():
    print("=" * 60)
    print("BENCHMARK PROGRAMU OBLICZAJĄCEGO LICZBĘ PI")
    print("=" * 60)
    ...
    colors = ['#2E86AB', '#A23B72', '#F18F01']  # Kolory dla różnych linii
    markers = ['o', 's', '^']
    ...
    plt.title('Wydajność obliczania liczby PI - zależność od liczby wątków', 
